...
#Пароль хешируется с использованием функции generate_password_hash из Flask. Это обеспечивает безопасное хранение пароля в базе данных.
    ...
    conn = dbConnect()
    cur = conn.cursor()
#Устанавливается соединение с базой данных и создается курсор. Затем выполняется запрос на проверку уникальности имени пользователя и, если проверка проходит успешно, 
#происходит вставка нового пользователя в базу данных
...
#Затем проверяется введенный пароль с хешированным паролем в базе данных с использованием функции check_password_hash из Flask. Если пароль верен, то аутентификация считается успешной.
            if check_password_hash(hashPassword, password):
                ...
                errors.append("Неправильный логин или пароль")
                return render_template("logins.html", errors=errors)
#В случае возникновения исключения при выполнении запроса к базе данных, добавляется сообщение об ошибке в список errors. Это может произойти, например, если произошла ошибка в самом SQL-запросе 
...
@lab5.route('/lab5/like_article/<int:article_id>')#Этот маршрут ожидает целочисленный параметр article_id
#Если в сессии отсутствует идентификатор пользователя (user_id), происходит перенаправление на страницу логина.
def like_article(article_id):
    if 'user_id' not in session:
        return redirect('/lab5/logins')
#Извлекается идентификатор пользователя из объекта сессии. Предполагается, что при успешной аутентификации пользователя в системе, его идентификатор сохраняется в сессии
    user_id = session['user_id']
#подключение
    conn = dbConnect()
    cur = conn.cursor()
#Выполняется запрос к базе данных для проверки, ставил ли пользователь лайк этой статье ранее. Результат сохраняется в previous_likes.
    try:
        # Проверка, ставил ли пользователь лайк ранее
#Запрос выбирает количество лайков (likes) из таблицы articles, где идентификатор статьи (id) равен значению article_id, и идентификатор пользователя (user_id) равен значению user_id. Знаки %s являются параметрами запроса, и значения article_id и user_id подставляются в эти места из кортежа, переданного вторым аргументом.
        cur.execute("SELECT likes FROM articles WHERE id = %s AND user_id = %s;", (article_id, user_id))#после выполнения этого метода, можно использовать другие методы, такие как fetchone() 
        previous_likes = cur.fetchone()
#cur.fetchone() возвращает одну строку из результата выполнения запроса в виде кортежа. В вашем случае, если запрос возвращает одну колонку, результат будет представлен кортежем из одного элемента.
#Эта часть кода проверяет, был ли ранее поставлен лайк для указанной статьи и пользователя. Если previous_likes не равно None, и 
#первый элемент кортежа (т.е., previous_likes[0]) также не равен None, это означает, что пользователь уже поставил лайк, и выполняется перенаправление 
#пользователя, например, на другую страницу.
...
#Этот SQL-запрос на языке Python используется для обновления значения столбца "likes" в таблице "articles". Значение "likes" увеличивается на 1 
#для строки, где значение столбца "id" соответствует переданному article_id. Функция COALESCE используется для обработки случаев, когда "likes" 
#может быть NULL, заменяя его на 0 перед увеличением. 
            cur.execute("UPDATE articles SET likes = COALESCE(likes, 0) + 1 WHERE id = %s;", (article_id,))
#conn.commit(): Этот вызов фиксирует изменения в базе данных. Без этой операции изменения не станут постоянными.
            conn.commit()
    finally:
        # Закрытие курсора и соединения с базой данных
        dbClose(cur, conn)
